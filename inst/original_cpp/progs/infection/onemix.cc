...
int main(int argc, char *argv[])
{
	...
	// Initialize parameters and read command line options.
		...
		int npar = 8;
		int hilo = 0;
	//	double param[] = {0.00001, 0.05, 1.0, -6, 0, 0.01, 0, 0.005};
		double *param;

		switch(argc)
		{
		case 5: sscanf(argv[4],"%d",&verbose);
		case 4: sscanf(argv[3],"%d",&hilo);
		...
		if (hilo > 0)
		{
			double p[] = {0.99, 0.99, 10.0, -1.0, 0, 1.0, 0, 0.99};
			param = p;
			cerr <<  "HI";
		}
		else
		{
			double p[] = {0.01, 0.01, 0.00001, -10.0, 0, 0.00001, 0, 0.01};
			param = p;
			cerr <<  "LO";
		}
	...
	// Run sampler and output parameter values.
  		...
  		for (int i=0; i<nsims; i++)
  		{
			...
			mps->sampleImportation(max,100,1,1,1,1);
			gps->sampleColonization(max,1,1);
			gps->sampleDecolonization(max,1,1);
