...
int main(int argc, char *argv[])
{
	...
	// Set simulation options from command line.
		...
		case 5: sscanf(argv[4],"%d",&nburn);
		case 4: sscanf(argv[3],"%d",&seed);
		case 3: sscanf(argv[2],"%d",&nsims);
		case 2: modfile.open(argv[1]);
			break;
		default:
			cerr << "Usage: runMCMC modelfile [nsims|100] [seed|0] [nburn|0] [verbose|0] [outputfinal|0] [outputparam|1] [outputwaic|0] [nmetro|10]\n";
			exit(1);
		}
	...
	// Read model from model specification file.
		...
		LTCFModel *model = readLTCF(modfile,l,nmetro,forward,cheat);

	// Set time origin of model.

		LTCFInColParams *icp = (LTCFInColParams *) model->getInColParams();
	...
	// Make and runsampler.
		...
		for (int i=0; i<nburn; i++)
		{
			if (!cheat)
				mc->sampleEpisodes();
		// Assume that burn in uses a good first guess
		// at parameter values, so don't update them
		// until the sampling stage.
		// This is to push the reconstructed histories
		// towards a reasonable point before sampling models.
		// Or, don't bother, and uncomment this line.
			mc->sampleModel();
		}
