...
protected:
	...
	static void putProposal(UnitLinkedModel *mod, EpisodeHistory *h, int nsim, double *times, int *states)
	{
		...
		if (mod->getNStates() == 2)
		{
			if (states[0] == 1)
			{
				h->proposeSwitch(mod->makeHistLink(f,u,times[0],p,acquisition,0));
			}
		}
		if (mod->getNStates() == 3)
		{
			...
			{
				h->proposeSwitch(mod->makeHistLink(f,u,times[0],p,acquisition,0));
				h->proposeSwitch(mod->makeHistLink(f,u,times[0],p,progression,0));
			}
		}

		for (int i = 1; i < nsim ; i++)
			h->proposeSwitch(mod->makeHistLink(f,u,times[i],p,eventOutOfState(mod->getNStates(),states[i-1]),1));
	}
...
// step. But it needs work.
		...
		if (mod->getNStates() == 2)
		{
			...
			case insitu2:
			case admission2:
				eh->proposeSwitch(mod->makeHistLink(f,u,admit,p,acquisition,0));
				break;
			default:
				break;
			}
